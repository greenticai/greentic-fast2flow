...
1. `rustfmt --check` (edition 2021) on all staged `.rs` files — fails if any isn't formatted. Run `cargo fmt --all` and re-stage.
...
3. `cargo sort --workspace` when a `Cargo.toml` is staged — alphabetizes dependency lists and re-stages.
